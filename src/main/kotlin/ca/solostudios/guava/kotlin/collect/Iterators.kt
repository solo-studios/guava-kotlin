...
/**
 * Returns the number of elements remaining in `this`. The iterator will
 * be left exhausted: its [hasNext][Iterator.hasNext] method will return
 * `false`.
 *
 * @see Iterators.size
 */
...
/**
 * Traverses an iterator and removes every element that belongs to
 * the provided collection. The iterator will be left exhausted:
 * its [hasNext][Iterator.hasNext] method will return `false`.
 *
 ...
 * from the iterator. The iterator will be left exhausted: its
 * [hasNext][Iterator.hasNext] method will return `false`.
 *
...
/**
 * Traverses an iterator and removes every element that does not belong
 * to the provided collection. The iterator will be left exhausted:
 * its [hasNext][Iterator.hasNext] method will return `false`.
 *
...
/**
 * Determines whether two iterators contain equal elements in the
 * same order. More specifically, this method returns `true` if
 * [this] and [other] contain the same number of elements and every
 * element of [this] is equal to the corresponding element of [other].
 *
...
/**
 * Returns a string representation of [this], with the format
 * `[e1, e2, ..., en]`. The iterator will be left exhausted:
 * its [hasNext][Iterator.hasNext] method will return `false`.
 *
 ...
 * exhausted: its [hasNext][Iterator.hasNext] method will return `false`.
 *
 * @param T the type of the elements
 * @return a newly-allocated array into which all the elements of the
 *         iterator have been copied
 * @see Iterators.toArray
 */
public inline fun <reified T : Any> Iterator<T>.toTypedArray(): Array<out T> {
    return Iterators.toArray(this, T::class.java) as Array<out T>
}

/**
 * Adds all elements in `iterator` to [this]. The iterator will be left
 * exhausted: its [hasNext][Iterator.hasNext] method will return `false`.
 *
 ...
 * the specified object. The iterator will be left exhausted: its
 * [hasNext][Iterator.hasNext] method will return `false`.
 *
 ...
 * [this].
 *
 * The iterator's [hasNext][Iterator.hasNext] method returns `true` until
 * [this] is empty.
 *
 ...
 * [this].
 *
 * The returned iterator supports [remove][MutableIterator.remove] if
 * the provided iterator does. After [remove][MutableIterator.remove] is
 ...
 * [remove][MutableIterator.remove] is called, subsequent cycles omit
 * the removed element, but `elements` does not change. The iterator's
 ...
 * iterates across the elements in [this], followed by the elements
 * in [other]. The source iterators are not polled until necessary.
 *
 ...
 * in [other]. The source iterators are not polled until necessary.
 *
 * The returned iterator supports [remove][MutableIterator.remove] when the
 ...
 * iterates across the elements in [this], followed by the elements in [a],
 * followed by the elements in [b]. The source iterators are not polled
 * until necessary.
 *
 * @see Iterators.concat
 */
 ...
 * Combines two iterators into a single iterator. The returned iterator
 * iterates across the elements in [this], followed by the elements in [a],
 * followed by the elements in [b], followed by the elements in [c]. The
...
/**
 * Combines two iterators into a single iterator. The returned iterator
 * iterates across the elements in [this], followed by the elements in
 * [iterators] sequentially. The source iterators are not polled until
 * necessary.
 *
 * @see Iterators.concat
 */
...
/**
 * Combines two iterators into a single iterator. The returned iterator
 * iterates across the elements in [this] sequentially. The source
...
/**
 * Concats a varargs array of iterators without making a defensive copy of
 * the array.
 *
 ...
 * c], [d, e]]` -- an outer iterator containing two inner lists of three
 ...
 * padding the final iterator with null values if necessary. For example,
 * partitioning an iterator containing `[a, b, c, d, e]` with a partition
 * size of 3 yields `[[a, b, c], [d, e, null]]` -- an outer iterator
...
/**
 * Returns `true` if one or more elements returned by [this] satisfy the
 * given [predicate].
 *
 * @see Iterators.any
 */
...
/**
 * Returns the first element in [this] that satisfies the given
 * [predicate]; use this method only when such an element is
 * known to exist. The iterator will be left exhausted: its
 * [hasNext][Iterator.hasNext] method will return `false`. If it is
 * possible that *no* element will match, use [Iterator.tryFind] or
 * [Iterator.find] instead.
 *
 ...
 * satisfies the given predicate, if such an element exists. If no such
 * element is found, an empty [Optional] will be returned from this method
 * and the iterator will be left exhausted: its [hasNext][Iterator.hasNext]
 ...
 * provided [predicate], or `-1` if the Iterator has no such elements.
 *
 * More formally, returns the lowest index `i` such that
 * `predicate.apply(Iterators.get(iterator, i))` returns `true`, or `-1` if
 * there is no such index.
 *
 * If `-1` is returned, the iterator will be left exhausted: its
 * [hasNext][Iterator.hasNext] method will return `false`. Otherwise, the
 * iterator will be set to the element which satisfies the [predicate].
 *
 ...
 *
 * The returned iterator supports [remove][MutableIterator.remove] if
 * [this] does. After a successful [remove][MutableIterator.remove]
 * call, [this] no longer contains the corresponding element.
 *
 * @see Iterators.transform
 */
...
/**
 * Advances [this]` `position + 1` times, returning the element at the
 ...
 *         than or equal to the number of elements remaining in [this].
 * @see Iterators.get
 */
...
/**
 * Advances [this]` `position + 1` times, returning the element at the
 * [position]th position or a default value otherwise.
 *
 * @param position position of the element to return
...
/**
 * Returns the next element in [this] or [defaultValue] if the iterator is
 * empty. The [Iterables] analog to this method is [Iterables.getFirst].
 *
 * @param defaultValue the default value to return if the iterator is empty
...
/**
 * Advances [this] to the end, returning the last element or [defaultValue]
 * if the iterator is empty and [defaultValue] is not `null`.
 *
 * @return the last element of [this]
 * @throws NoSuchElementException if the iterator is empty and
...
/**
 * Calls [next][Iterator.next] on [this], either [numberToAdvance] times
 * or until [hasNext][Iterator.hasNext] returns `false`, whichever comes
 * first.
 *
 ...
 * Returns a view containing the first [limitSize] elements of [this].
 * If [this] contains fewer than [limitSize] elements, the returned view
 * contains all of its elements. The returned iterator supports `remove()`
 * if `iterator` does.
 *
 * @param limitSize the maximum number of elements in the returned iterator
 * @throws IllegalArgumentException if [limitSize] is negative
...
/**
 * Returns a view of the supplied [this] that removes each element from the
 * supplied [this] as it is returned.
 *
 * The provided iterator must support [remove][MutableIterator.remove] or
 ...
 * returned iterator is a view of the array; subsequent changes to the
 * array will be reflected in the iterator.
 *
 * **Note:** It is often preferable to represent your data using a
 * collection type, for example using [Arrays.asList], making this method
 * unnecessary.
 *
 ...
 * [Enumeration] as an [Iterable] is impossible. However, the contents
 * can be *copied* into a collection using [Collections.list].
 *
 ...
 * [Collections.enumeration] (if you have a [Collection]),
 ...
 *
 * Calls to the [peek][PeekingIterator.peek] method with no intervening
 * calls to [Iterator.next] do not affect the iteration, and hence return
 * the same object each time. A subsequent call to [Iterator.next]
 * is guaranteed to return the same object again. For example:
 * ```kotlin
 ...
 * performed by the iterator's own [PeekingIterator.remove] method) will
 ...
 *
 * Note: If the given iterator is already a [PeekingIterator], it *might*
 * be returned to the caller, although this is neither guaranteed to occur
 * nor required to be consistent. For example, this method *might* choose
 * to pass through recognized implementations of [PeekingIterator] when the
 * behavior of the implementation is known to meet the contract guaranteed
 * by this method.
 *
 * There is no [Iterable] equivalent to this method, so use this method to
 ...
 * @receiver The backing iterator. The [PeekingIterator] assumes ownership
 *         of this iterator, so users should cease making direct calls to
...
/**
 * Returns an iterator over the merged contents of all given [this],
 * traversing every element of the input iterators. Equivalent entries will
 * not be de-duplicated.
 *
 * Callers must ensure that the source [Iterator]s are in non-descending
 * order as this method does not sort its input.
 *
 * For any equivalent elements across all of [this], it is undefined which
