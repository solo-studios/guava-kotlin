 ...
 * collection.
 *
 * This method calls [MutableCollection.removeAll] if [this] is a
 ...
 * provided collection.
 *
 * This method calls [MutableCollection.retainAll] if [this] is a
 ...
 * [predicate] is dependent on [this].
 *
 * **Java 8 users:** if [this] is a [Collection], use
 * `this.removeIf(predicate)` instead.
 *
 ...
 * order. More specifically, this method returns `true` if [this]
 * and [other] contain the same number of elements and every element
 * of [other] is equal to the corresponding element of [other].
 *
...
/**
 * Returns a string representation of [this], with the
 * format `[e1, e2, ..., en]` (that is, identical to
 * [Arrays][java.util.Arrays]`.toString(Iterables.toArray(iterable))`).
 ...
 */
public inline fun <reified T : Any> Iterable<T>.toTypedArray(): Array<out T> {
 ...
 *
 * @return `true` if `collection` was modified as a result of this
 *         operation.
 ...
 * iterable is a [Multiset] or [Set].
 *
 * **Java 8 users:** In most cases, the [Stream] equivalent of this method
 * is `stream.filter(element::equals).count()`. If `element` might be
 * null, use `stream.filter(Predicate.isEqual(element)).count()` instead.
 *
 ...
 * of [this].
 *
 * That iterator supports [remove][MutableIterator.remove] if
 * `iterable.iterator()` does. After [remove][MutableIterator.remove] is
 * called, subsequent cycles omit the removed element, which is no longer
 * in [this]. The iterator's [hasNext][Iterator.hasNext] method returns
 * `true` until [this] is empty.
 *
 * **Warning:** Typical uses of the resulting iterator may produce an
 * infinite loop. You should use an explicit `break` or be certain that you
 * will eventually remove all the elements.
 ...
 * elements.
 *
 * After [remove][MutableIterator.remove] is invoked on a generated
 * iterator, the removed element will no longer appear in either
 * that iterator or any other iterator created from the same
 * source iterable. That is, this method behaves exactly as
 * `Iterables.cycle(Lists.newArrayList(elements))`. The iterator's
 * [hasNext][Iterator.hasNext] method returns `true` until all of the
 ...
 *
 * **Java 8 users:** If passing a single element `e`, the [Stream]
 * equivalent of this method is `Stream.generate(() -> e)`. Otherwise,
 * put the elements in a collection and use `Stream.generate(() ->
 * collection).flatMap(Collection::stream)`.
 *
 * @see Iterables.cycle
 */
...
/**
 * Combines two iterables into a single iterable. The returned iterable
 * has an iterator that traverses the elements in [this], followed by
 * the elements in [other]. The source iterators are not polled until
 * necessary.
 *
 * **Java 8 users:** The [Stream] equivalent of this method is
 ...
 * [remove][MutableIterator.remove] when the
 ...
 * has an iterator that traverses the elements in [this], followed by the
 * elements in [a], followed by the elements in [b]. The source iterators
 * are not polled until necessary.
 *
 * **Java 8 users:** The [Stream] equivalent of this method is
 * `Stream.concat(this, b, c)`.
 *
 * @see Iterables.concat
 */
...
/**
 * Combines four iterables into a single iterable. The returned iterable
 * has an iterator that traverses the elements in [this], followed by
 * the elements in [a], followed by the elements in [b], followed by the
...
/**
 * Combines multiple iterables into a single iterable. The returned
 * iterable has an iterator that traverses the elements in [this], followed
 * by the elements of each iterable in [iterables] sequentially. The source
...
/**
 * Combines multiple iterables into a single iterable. The returned
 * iterable has an iterator that traverses the elements in [this]
 * sequentially. The source iterators are not polled until necessary.
 *
 * **Java 8 users:** The [Stream] equivalent of this method is
 ...
 * containing `[a, b, c, d, e]` with a partition size of 3 yields `[[a, b,
 * c], [d, e]]` -- an outer iterable containing two inner lists of three
 * and two elements, all in the original order.
 *
 * **Note:** The current implementation eagerly allocates storage for
 * [size] elements. As a consequence, passing values like [Int.MAX_VALUE]
 ...
 * padding the final iterable with null values if necessary. For example,
 * partitioning an iterable containing `[a, b, c, d, e]` with a partition
 * size of 3 yields `[[a, b, c], [d, e, null]]` -- an outer iterable
 * containing two inner lists of three elements each, all in the original
 * order.
 *
 * Iterators returned by the returned iterable do not support the
 * [remove][MutableIterator.remove] method.
 *
 * @param size the desired size of each partition
 * @return an iterable of unmodifiable lists containing the elements of
 *         [this] divided into partitions (the final iterable may have
...
/**
 * Returns a view of [this] containing all elements that satisfy the input
...
/**
 * Returns a view of [this] containing all elements that are of the type
 * [T].
 *
 * **[Stream] equivalent:**
 * `stream.filter(type::isInstance).map(type::cast)`. This does perform
 * a little more work than necessary, so another option is to insert an
...
/**
 * Returns `true` if every element in [this] satisfies the predicate. If
 * [this] is empty, `true` is returned.
 *
...
/**
 * Returns the first element in [this] that satisfies the given predicate;
 * use this method only when such an element is known to exist. If it is
 * possible that *no* element will match, use [tryFind] or [find] instead.
 *
 * **[Stream] equivalent:** `stream.filter(predicate).findFirst().get()`
 *
 * @throws NoSuchElementException if no element in [this] matches the given
 *         predicate and [defaultValue] is `null`.
 * @see Iterables.find
 */
...
/**
 * Returns an [Optional] containing the first element in [this] that
 * satisfies the given predicate, if such an element exists.
 *
 * **Warning:** avoid using a [predicate] that matches `null`. If `null` is
 * matched in [this], a NullPointerException will be thrown.
 *
...
/**
 * Returns the index in [this] of the first element that satisfies the
 * provided [predicate], or `-1` if the Iterable has no such elements.
 *
 * More formally, returns the lowest index `i` such that
 * `predicate.apply(Iterables.get(iterable, i))` returns `true`, or `-1` if
 ...
 *
 * The returned iterable's iterator supports
 * [remove][MutableIterator.remove] if [this]'s iterator does. After a
 * successful [remove][MutableIterator.remove] call, [this] no longer
 ...
 *         than or equal to the size of [this]
 * @see Iterables.get
 */
...
/**
 * Returns the element at the specified position in an iterable or a
 * default value otherwise.
 * * **[Stream] equivalent:** `stream.skip(position).findFirst().get()`
 *   (throws [NoSuchElementException] if out of bounds)
 *
 * @param position position of the element to return
 * @param defaultValue the default value to return if [position] is greater
 *         than the size of [this]
 * @return the element at the specified position in [this], or
 *         [defaultValue] if [this] produces fewer that `position + 1`
 *         elements and [defaultValue] is not `null`.
 * @throws IndexOutOfBoundsException if [position] is negative, or if
 *         [position] is greater than or equal to the number of elements
 *         remaining in [this] *and* [defaultValue] is `null`.
 * @see Iterables.get
 */
@Throws(IndexOutOfBoundsException::class)
...
/**
 * Returns the first element in [this] or `defaultValue` if the iterable is
 * empty. The [Iterators] analog to this method is [Iterators.getNext].
 *
 * If no default value is desired (and the caller instead
 * wants a [NoSuchElementException] to be thrown), it is
 * recommended that `iterable.iterator().next()` is used instead.
 *
 * To get the only element in a single-element [this], consider using
 * [ ][.getOnlyElement] or [.getOnlyElement] instead.
 *
...
/**
 * Returns the last element of [this] or `defaultValue` if the iterable
 * is empty. If [this] is a [List] with [RandomAccess] support, then this
 * operation is guaranteed to be `O(1)`.
 *
 * **[Stream] equivalent:**
 * [Streams.findLast(stream).get()][Streams.findLast]
 *
 * @param defaultValue the value to return if [this] is empty
 * @return the last element of [this] or the default value
 * @throws NoSuchElementException if the iterable is empty and the default
 *         value is `null`
...
/**
 * Returns a view of [this] that skips its first [numberToSkip] elements.
 * If [this] contains fewer than [numberToSkip] elements, the returned
 * iterable skips all of its elements.
 *
 * Modifications to the underlying [Iterable] before a call to
 * [`iterator()`][Iterable.iterator] are reflected in the returned
 * iterator. That is, the iterator skips the first [numberToSkip] elements
 * that exist when the `Iterator` is created, not when [skip] is called.
 *
 ...
 *
 * The returned iterable's iterator supports
 * [`remove()`][MutableIterator.remove] if the iterator of
 * the underlying iterable supports it. Note that it is *not*
 * possible to delete the last skipped element by immediately
 * calling [`remove()`][MutableIterator.remove] on that
 * iterator, as the [MutableIterator] contract states that a
 * call to [`remove()`][MutableIterator.remove] before a call to
 * [`next()`][Iterable] will throw an [IllegalStateException].
 *
 * **[Stream] equivalent:** [Stream.skip]
 *
 * @see Iterables.skip
...
/**
 * Returns a view of [this] containing its first `limitSize` elements.
 * If [this] contains fewer than `limitSize` elements, the returned view
 * contains all of its elements. The returned iterable's iterator supports
 * `remove()` if [this]'s iterator does.
 *
 ...
 * [Iterator] through [Iterators.consumingIterator].
 *
 * Note: If [this] is a [Queue], the returned iterable will get entries
 * from [Queue.remove] since [Queue]'s iteration order is undefined.
 * Calling [Iterator.hasNext] on a generated iterator from the returned
 * iterable may cause an item to be immediately dequeued for return on a
 * subsequent call to [Iterator.next].
 *
 * @return a view of the supplied iterable that wraps each generated
 *         iterator through
 *         [`Iterators.consumingIterator(iterator)`][Iterators.consumingIterator];
 ...
 *
 * There is no precise [Iterator] equivalent to this method, since one can
 * only ask an iterator whether it has any elements *remaining* (which one
 * does using [Iterator.hasNext]).
 *
...
/**
 * Returns an iterable over the merged contents of all given [Iterable]s in
 * [this]. Equivalent entries will not be de-duplicated.
 *
 * Callers must ensure that the source [Iterable]s are in non-descending
 * order as this method does not sort its input.
 *
 * For any equivalent elements across all [this], it is undefined which
