...
/**
 * Returns every possible list that can be formed by choosing
 * one element from each of the given lists in order; the "n-ary
 ...
 *
 * *Performance notes:* while the cartesian product of lists of size `m, n,
 * p` is a list of size `m x n x p`, its actual memory consumption is much
 * smaller. When the cartesian product is constructed, the input lists are
 * merely copied. Only as the resulting list is iterated are the individual
 * lists created, and these are not retained after iteration.
 *
 ...
 *         would be greater than [Integer.MAX_VALUE]
 * @throws NullPointerException any one of the [List]s in [this], or any
 ...
 * @throws IllegalArgumentException if the size of the cartesian product
 *         would be greater than [Integer.MAX_VALUE]
 * @throws NullPointerException if [lists], any one of the [List]s in
...
/**
 * Returns a list that applies [function] to each element of [this]. The
 * returned list is a transformed view of [this]; changes to [this] will be
 ...
 * be applied many times for bulk operations like [List.contains] and
 * [List.hashCode]. For this to perform well, [function] should be fast. To
 * avoid lazy evaluation when the returned list doesn't need to be a view,
 * copy the returned list into a new list of your choosing.
 *
 * If [this] implements [RandomAccess], so will the returned list. The
 * returned list is threadsafe if the supplied list and function are.
 *
 * If only a `Collection` or `Iterable` input is available, use
 * [Collection.transform] or [Iterable.transform].
 *
 * **Note:** serializing the returned list is implemented by serializing
 * [this], its contents, and [function] -- *not* by serializing the
 * transformed values. This can lead to surprising behavior, so serializing
 * the returned list is **not recommended**. Instead, copy the list using
 * [ImmutableList.copyOf] (for example), then serialize the copy. Other
 * methods similar to this do not implement serialization at all for this
 * reason.
 *
 * **Java 8 users:** many use cases for this method are better addressed by
 * [Stream.map][Stream.map]. This method is not being deprecated, but we
 ...
 * items cannot be stored in the returned list. The `add`, `addAll` and
 ...
 * size (the final list may be smaller). For example, partitioning a list
 * containing `[a, b, c, d, e]` with a partition size of 3 yields `[[a, b,
 * c], [d, e]]` -- an outer list containing two inner lists of three and
 * two elements, all in the original order.
 *
 * The outer list is unmodifiable, but reflects the latest state of the
 * source list. The inner lists are sublist views of the original list,
 * produced on demand using [List.subList], and are subject to all the
...
/**
 * Returns a view of the specified string as an immutable list of [Char]
 * values.
 *
...
/**
 * Returns a view of the specified [CharSequence] as a `List<Character>`,
 * viewing [this] as a sequence of Unicode code units. The view does not
 * support any modification operations, but reflects any changes to the
